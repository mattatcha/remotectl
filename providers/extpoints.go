...
var Providers = &providerExt{
	newExtensionPoint(new(Provider)),
}
...
func (ep *providerExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *providerExt) Register(extension Provider, name string) bool {
	return ep.register(extension, name)
}

func (ep *providerExt) Lookup(name string) Provider {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
...
func (ep *providerExt) Select(names []string) []Provider {
...
func (ep *providerExt) All() map[string]Provider {
...
func (ep *providerExt) Names() []string {
	var names []string
	for k := range ep.all() {
